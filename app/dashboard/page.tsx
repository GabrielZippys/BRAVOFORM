...
import { Company, Department, Form, FormField, FormResponse } from '@/types';
...
export default function DashboardPage() {
    ...
    // Busca departamentos quando uma empresa é selecionada
    useEffect(() => {
    ...
    // Busca respostas de formulários
    useEffect(() => {
        const fetchResponses = async () => {
            ...
            try {
                ...
                const q = query(collection(db, "form_responses"));
                const querySnapshot = await getDocs(q);
    ...
    const kpiData = useMemo(() => {
        ...
        const filteredByTime = filteredResponses.filter(r => {
            ...
            switch (timeFilter) {
                case 'day': 
                    return responseDate.getDate() === now.getDate() && 
                ...
                case 'month':
                    return responseDate.getMonth() === now.getMonth() && 
                           responseDate.getFullYear() === now.getFullYear();
                case 'year':
                    return responseDate.getFullYear() === now.getFullYear();
                default:
                    return true;
            }
        });
        ...
        return {
            ...
            completionRate: filteredForms.length > 0 ? 
                (filteredResponses.length / filteredForms.length) : 0
        };
    ...
    // Ícones para os cartões de estatísticas
    const icons = {
        responses: (
        ...
        <div className={styles.dashboardContainer}>
            <div className={styles.header}>
                ...
                <div className={styles.timeFilters}>
                    <button 
                        className={`${styles.timeFilter} ${timeFilter === 'day' ? styles.active : ''}`}
            ...
            <div className={styles.statsGrid}>
                <StatCard 
                    ...
                    value={kpiData.responsesInPeriod} 
                    icon={icons.responses}
                    isLoading={loading.responses || loading.forms} 
                />
                <StatCard 
                    title="Formulários Criados" 
                    value={kpiData.totalForms} 
                    icon={icons.forms}
                ...
                <StatCard 
                    ...
                    value={`${(kpiData.completionRate * 100).toFixed(1)}%`} 
                    icon={icons.rate}
                    isLoading={loading.responses || loading.forms} 
                />
            </div>
            ...
            <div className={styles.filtersContainer}>
                ...
                <div className={styles.filterGroup}>
                    ...
                    <select 
                        ...
                        className={styles.filterSelect}
                        disabled={!selectedCompanyId || selectedCompanyId === 'all' || loading.departments}
                    >
                ...
                <div className={styles.chartContainer}>
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={submissionsByDay}>
                            <CartesianGrid strokeDasharray="3 3" stroke="rgba(177, 143, 66, 0.2)" />
                            <XAxis dataKey="date" stroke="var(--text-ivory)" />
                            <YAxis stroke="var(--text-ivory)" />
                            <Tooltip 
                                contentStyle={{ 
                            ...
                            <div key={form.id} className={styles.formItem}>
                                <div className={styles.formInfo}>
                                    ...
                                    <p className={styles.formMeta}>
                                        {form.createdAt?.toDate().toLocaleDateString('pt-BR')} • 
                                        {form.status === 'active' ? ' Ativo' : 
                                         form.status === 'draft' ? ' Rascunho' : 
                                         form.status === 'archived' ? ' Arquivado' : ' Status Desconhecido'}
                                    </p>
                                </div>
                            ...
                            <div key={response.id} className={styles.responseItem}>
                                <div className={styles.responseInfo}>
                                    ...
                                    <p className={styles.responseMeta}>
                                        {response.submittedAt?.toDate().toLocaleString('pt-BR')} • 
                                        {response.collaboratorName || 'Usuário Desconhecido'}
                                    </p>
                                </div>
                                <div className={styles.responseStatus}>
                                    {response.completed ? 'Completo' : 'Parcial'}
